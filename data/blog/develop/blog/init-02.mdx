...
title: '블로그개발기 (2)'
date: '2022-06-29'
tags: ['react', 'Semantic-UI', 'React-Bootstrap']
draft: true
...
(참고로 최근에는 Semantic UI가 아닌 [Fomantic-UI](https://fomantic-ui.com/) 에서 버전업이 진행되고 있는 것 같다.)

#### SEO
화면 UI도 정말 심플하지만 완성이 되었고 홈화면, 목록화면(전체 / 카테고리별 목록), 게시글 화면 등 라우팅 처리까지 완료되었다.
앞으로 한 단계, SEO 만이 남았으나 이전에도 해보았고 무난하게 진행되리라 생각했다. 
...
const create = async () => {
    ...
    const postUrlSet = p.map(post => `    <url>
...
이 부분을 보완하기 위해 **react-helmet** 을 사용해 페이지 별로 <head> 영역 내의 정보들을 다르게 구성하였고 og 관련 meta 정보들을 추가하였다.
...
- 참고 URL
  - 카카오 캐시 삭제
  - 페이스북 Open Graph Debugger
  - Open Graph Debuuger
<..cheesepaninim.netlify.app => 화면들 첨부>

---
생각보다 내용이 길어져서 현재 블로그에 관한 내용은 [블로그개발기 (3)](https://cheesepaninim.vercel.app/blog/develop/blog/init-03)에서 이어가려 한다.
